/**
 * IMPROVED PERSONA CARD - De-slopped Version
 *
 * Changes from original:
 * 1. ❌ Removed hover:scale-* (AI slop indicator)
 * 2. ✅ Added left border accent (spiritual/unique)
 * 3. ✅ Simplified rounded corners (only image)
 * 4. ✅ Minimal hover effects (border color change)
 * 5. ✅ Clean typography hierarchy
 * 6. ✅ Image scales on hover, not card
 *
 * To use: Replace contents of PersonaCard.jsx with this
 */
...
export default function PersonaCard({ persona, onEdit, onLikeChange }) {
  ...
  // Chakra-inspired category colors (spiritual theme)
  const getCategoryBorderColor = (category) => {
    const colors = {
      'Relationships': 'border-l-amber-600',     // Warm gold
      'Wellness': 'border-l-emerald-600',        // Nature green
      'Career': 'border-l-blue-600',             // Professional blue
      'Lifestyle': 'border-l-purple-600',        // Creative purple
      'Finance': 'border-l-green-700',           // Money green
      'Health': 'border-l-teal-600',             // Healing teal
      'Spiritual': 'border-l-indigo-600',        // Mystical indigo
      'Parenting': 'border-l-orange-600',        // Nurturing orange
      'Legal': 'border-l-slate-700'              // Authoritative gray
    }
    return colors[category] || 'border-l-black'
  }
  ...
  return (
    <div
      className={`
        ...
        hover:shadow-md hover:border-l-amber-500
    ...
    >
      ...
      <div className="p-3 space-y-1">
        <div className="flex items-start justify-between gap-2">
          {/* Name - Uses display font if configured */}
 ...
 *
 * 1. Add these colors to tailwind.config.js:
 *    colors: {
 *      'amber': { 500: '#F59E0B', 600: '#D97706' },
 *    }
 *
 * 2. Install a display font (optional):
 *    - Crimson Text (serif, elegant)
 *    - Libre Baskerville (classic)
 *    - Cormorant (spiritual vibe)
 *
 * 3. The key differences:
 *    - Left border provides visual hierarchy
 *    - No card scaling (only image scales)
 *    - Clean, minimal hover states
 *    - Gold accent color (#D4AF37) for likes
 *    - Sharp corners except image
 */
