...
const GUEST_LAST_PROMPT_COUNT_KEY = 'esperit_last_prompt_count'
const GUEST_PREMIUM_DISMISSED_KEY = 'esperit_premium_prompt_dismissed'

const SIGNIN_LIMIT = 100
const PREMIUM_LIMIT = 150

// Soft sign-in prompt pacing, in messages (not seconds).
const FIRST_PROMPT_AT_MESSAGES = 10
const PROMPT_EVERY_MESSAGES = 25
...
export function resetGuestMessageCount() {
  ...
  localStorage.removeItem(GUEST_LAST_PROMPT_COUNT_KEY)
  localStorage.removeItem(GUEST_PREMIUM_DISMISSED_KEY)
}

/**
 * Check whether to show the soft (dismissible) sign-in prompt.
 *
 * Driven by message count, not a wall-clock timer. The previous timer version
 * returned true on the very first call (no stored timestamp yet), so a brand-new
 * guest was asked to sign in on their first message — the worst possible moment,
 * and a measurable drop-off point for organic visitors.
 *
 * Now: stay silent until the visitor is invested (FIRST_PROMPT_AT_MESSAGES), then
 * ask again only occasionally (PROMPT_EVERY_MESSAGES).
 *
 * @returns {boolean} True if the soft sign-in prompt should be shown
 */
export function shouldShowTimeBasedPrompt() {
  ...
  // Hard limits (100/150) own the UX past this point — don't stack a soft prompt on top.
  if (count >= SIGNIN_LIMIT) return false

  // Let people actually experience the persona first.
  if (count < FIRST_PROMPT_AT_MESSAGES) return false

  const lastPromptCount = parseInt(
    localStorage.getItem(GUEST_LAST_PROMPT_COUNT_KEY) || '0',
    10
  )

  // First eligible prompt, or enough messages since the last one.
  if (lastPromptCount === 0) return true

  return count - lastPromptCount >= PROMPT_EVERY_MESSAGES
}

/**
 * Record that the soft sign-in prompt was just shown.
 * Stores the message count (the pacing signal) and the timestamp (kept for analytics).
 * Name retained for call-site compatibility.
 */
export function updateLastPromptTime() {
  if (typeof window === 'undefined') return

  localStorage.setItem(GUEST_LAST_PROMPT_COUNT_KEY, getGuestMessageCount().toString())
