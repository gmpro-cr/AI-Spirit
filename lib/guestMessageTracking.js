...
// - 20 messages: Compulsory sign-in (hard block)
// - 50 messages: Prompt for premium subscription (hard block)
...
export function shouldShowTimeBasedPrompt() {
  ...
  // Don't show time-based prompts if user hit 20 or 50 message limits
...
/**
 * Check if sign-in is compulsory (20 messages reached)
 * @returns {boolean} True if 20 messages reached
 */
...
/**
 * Check if message can be sent (enforces 20 and 50 message limits)
 * @returns {Object} { canSend: boolean, reason: string|null, message: string|null }
 */
export function canSendMessage() {
  const count = getGuestMessageCount()

  // Premium limit (50 messages) - hard stop
  if (count >= PREMIUM_LIMIT) {
    return {
      canSend: false,
      reason: 'premium_required',
      message: 'You\'ve reached your 50 message limit! Upgrade to Premium for unlimited access.'
    }
  }

  // Sign-in limit (20 messages) - hard stop
  if (count >= SIGNIN_LIMIT) {
    return {
      canSend: false,
      reason: 'signin_required',
      message: 'You\'ve reached your 20 message limit! Sign in to continue chatting.'
    }
  }
