...
/**
 * Generates a streaming persona response using Google Gemini AI
...
export async function* generatePersonaResponseStream(systemPrompt, messageHistory, contextString = null) {
  try {
    ...
    console.error('Gemini Stream API Error:', {
...
export async function generatePersonaResponse(systemPrompt, messageHistory, metadata = {}, contextString = null) {
  try {
    ...
    // Rate limit or quota exceeded - return technical error for fallback
    if (error.message?.toLowerCase().includes('quota') ||
      error.message?.toLowerCase().includes('rate') ||
      error.status === 429) {
      return {
        success: false,
        error: 'RATE_LIMIT_EXCEEDED', // Technical error code for fallback
