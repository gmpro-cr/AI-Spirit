 ...
 * @param {string|null} conversationId - The conversation ID (null for guest users' first message)
 * @returns {Promise<string|null>} Context string or null if not needed
 */
export async function getContextIfNeeded(conversationId) {
  // For guest users' first message, conversationId will be null - inject context
