...
// Remove words that are legitimate in educational/philosophical contexts
const allowedWords = [
  'sex',
  'sexual',
  'sexuality',
...
// Remove these words from the filter
allowedWords.forEach(word => {
  filter.removeWords(...word.split(' '))
})
...
const PII_PATTERNS = {
  ...
  // US format: 123-456-7890 | Indian mobile: 10 consecutive digits starting with 6-9
  phone: /\b(?:\d{3}[-.]?\d{3}[-.]?\d{4}|[6-9]\d{9})\b/,
  email: /\b[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}\b/i,
  // Indian Aadhaar: 12-digit number (often written with spaces: XXXX XXXX XXXX)
...
export function moderateContent(text) {
  ...
  // 2. Check for profanity - DISABLED to allow personas to respond authentically
  // Personas should be able to use any language appropriate to their character
  // The AI models (Gemini/Groq) have their own safety filters
