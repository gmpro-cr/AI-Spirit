...
export function extractMemoriesSimple(userMessage) {
    ...
    // Birthdate patterns
    if (msg.includes('born on') || msg.includes('birthday is') || msg.includes('birth date') || msg.includes('dob is')) {
        const dateMatch = userMessage.match(/(?:born on|birthday is|birth date is|dob is)\s*([\d\/\-]+(?:\s*[a-z]+)?[\d\/\-\s]*\d*)/i)
        if (dateMatch) {
            memories.push({ fact: `Birthday: ${dateMatch[1].trim()}`, category: 'birthdate' })
        }
    }

    // Birth time and place (for astrology personas)
    if (msg.includes('born at') || msg.includes('birth time') || msg.includes('birth place')) {
        const birthMatch = userMessage.match(/(?:born at|birth time is|birth place is)\s*([^.,!]+)/i)
        if (birthMatch) {
            memories.push({ fact: `Birth details: ${birthMatch[1].trim()}`, category: 'birthdate' })
        }
    }
