...
export default function ChatInterface({ persona, onSendMessage }) {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-64px)]">
      {/* Persona Header */}
      <div className="relative bg-gradient-to-br from-white/15 via-white/10 to-white/5 backdrop-blur-2xl border-2 border-white/40 rounded-3xl p-4 m-4 flex items-center space-x-4 shadow-[0_8px_32px_0_rgba(0,0,0,0.3)] before:absolute before:inset-0 before:rounded-3xl before:bg-gradient-to-tr before:from-white/25 before:via-transparent before:to-transparent before:opacity-60 before:pointer-events-none after:absolute after:inset-[1px] after:rounded-3xl after:bg-gradient-to-br after:from-transparent after:via-white/5 after:to-white/10 after:pointer-events-none">
        <div className="relative z-10 w-12 h-12 rounded-full bg-gradient-to-br from-white/30 via-white/20 to-white/10 backdrop-blur-md border-2 border-white/50 flex items-center justify-center text-xl text-white font-bold shadow-[inset_0_2px_4px_rgba(255,255,255,0.2)] shadow-[0_0_20px_rgba(255,255,255,0.2)]">
          {persona.name[0]}
        </div>
        <div className="relative z-10">
          <h2 className="font-semibold text-lg text-white drop-shadow-[0_2px_8px_rgba(255,255,255,0.3)]">{persona.name}</h2>
          <p className="text-white/90 text-sm">{persona.description}</p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-4">
        {messages.length === 0 && (
          <div className="text-center text-white/80 mt-8">
            <p className="mb-4 drop-shadow-sm">Start a conversation with {persona.name}</p>
            <div className="space-y-2">
              {persona.conversation_starters?.map((starter, idx) => (
                <button
                  key={idx}
                  onClick={() => onSendMessage(starter)}
                  className="relative block w-full bg-gradient-to-br from-white/15 via-white/10 to-white/5 backdrop-blur-2xl border-2 border-white/40 rounded-3xl p-3 hover:from-white/25 hover:via-white/20 hover:to-white/10 hover:border-white/60 shadow-[0_4px_16px_0_rgba(0,0,0,0.3)] hover:shadow-[0_4px_20px_0_rgba(255,255,255,0.2)] transition-all duration-300 text-left text-white before:absolute before:inset-0 before:rounded-3xl before:bg-gradient-to-tr before:from-white/25 before:via-transparent before:to-transparent before:opacity-50 before:pointer-events-none after:absolute after:inset-[1px] after:rounded-3xl after:bg-gradient-to-br after:from-transparent after:via-white/5 after:to-white/10 after:pointer-events-none"
                >
